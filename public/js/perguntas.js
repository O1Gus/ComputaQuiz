...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function iniciarQuiz() {
    perguntaAtual = 0;
    selecionadas = [];
    vidas = 3; // reinicia vidas quando começa novo quiz
  ...
  function mostrarPergunta() {
    ...
    // embaralha opções mantendo índice original
    ...
    botoes.forEach((btn, i) => {
      if (opcoesEmbaralhadas[i]) {
        ...
        btn.disabled = false; // reativa para a nova pergunta
      }
    });
  ...
  function verificarResposta(indexEscolhido) {
    const correta = perguntas[selecionadas[perguntaAtual]].correta;
    ...
    if (parseInt(indexEscolhido) !== correta) {
      ...
      botoes[currentIndex].disabled = true; // desativa a opção escolhida
      //alert(`Você errou! Restam ${vidas} vidas.`);
      ...
      if (vidas <= 0) {
        ...
        setTimeout(() => {
          ...
          iniciarQuiz();
        }, 1000); // espera animação antes de reiniciar
      ...
      return; // não avança de pergunta ainda
    }
